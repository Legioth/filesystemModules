// Slightly modified, original at https://unpkg.com/es-module-shims@0.4.5/dist/es-module-shims.js
...
(function () {
  ...
  async function resolve (id, parentUrl) {
    return resolveIfNotPlainOrUrl(id, parentUrl) || resolveModule(id, parentUrl);
  }
  ...
  async function getFileOrDir (parent, name) {
    try {
  ...
  async function doFetch (url) {
    if (url.startsWith("module://")) {
      let fsPath = url.substring("module://".length);
      let handle = await resolveFromFs(window.node_modules, fsPath);

      if (!handle) {
        throw Error(fsPath + " not found in the file system");
      }

      return crateFakeResponse(handle, url);
    } else {
      return fetch(url);
    }
  }

  async function crateFakeResponse (handle, url) {
    // Create something that looks like a fetch response to keep the rest of the code happy
    return {
      ok: true,
      headers: {
        get: name => {
          if ("content-type" === name) {
            if (handle.name.endsWith(".js")) {
              return "text/javascript";
            } else {
              throw Error("Unsupported file type for " + handle.name);
            }
          } else {
            throw Error("Unexpected header request: " + name);
          }
        }
      },
      text: async () => {
